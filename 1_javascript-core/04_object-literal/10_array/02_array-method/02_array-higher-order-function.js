...
// 기본적으로 문자열 정렬로 한자리수나 세자리수가 올바르지 않게 정렬 되는 모습을 보임
...
// Array.prototype.filter : 배열 요소 전체를 대상으로 콜백 함수 호출 후 반환 값이 true인 요소로만
// 구성 된 새로운 배열 반환
...
// 배열 내 요소 중 모든 값이 3보다 큰지 확인
result = [1, 5, 3, 2, 4].every(item => item > 3);
console.log(`result : ${result}`);
// 배열 내 요소 중 모든 값이 0보다 큰지 확인
result = [1, 5, 3, 2, 4].every(item => item > 0);
console.log(`result : ${result}`);

// Array.prototype.find : 배열을 순회하며 각 요소에 대하여 인자로 주어진 롤백 함수를 실행하여
